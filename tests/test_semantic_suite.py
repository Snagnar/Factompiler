...
class TestSemanticAnalyzer:
    ...
    def test_integer_literal_type(self):
        ...
        # Check that Signal type is created with integer value
        num_symbol = analyzer.symbol_table.lookup("num")
        assert num_symbol is not None
        assert isinstance(num_symbol.value_type, SignalValue)

    def test_input_signal_type(self):
        """Test type inference for input expressions."""
        code = """
        Signal signal_A = input(0);
        Signal iron_plates = input("iron-plate", 1);
        """
        analyzer = self.parse_and_analyze(code)
    ...
    def test_mixed_type_arithmetic(self):
        ...
        code = """
        Signal a = input(0);      # Signal
        Signal b = 10;            # Integer
        Signal mixed = a + b;     # Should allocate implicit type
        """
        analyzer = self.parse_and_analyze(code)
    ...
    def test_undefined_memory_error(self):
        ...
        code = """
        Signal value = read(undefined_memory);
        """
        analyzer = self.parse_and_analyze(code)
    ...
    def test_function_parameter_scoping(self):
        ...
        func test_scope(param) {
            Signal local_var = param + 1;
            return local_var;
        }
    
        Signal global_var = 10;
        """
        analyzer = self.parse_and_analyze(code)
    ...
    def test_multiple_errors_collection(self):
        ...
        code = """
        Signal x = undefined1 + undefined2;
        Signal y = another_undefined;
    ...
    def test_strict_type_checking(self):
        ...
        Signal a = input(0);
        Signal b = 10;
        Signal mixed = a + b;  # Should be error in strict mode
        """

        # Non-strict mode: should be warning
        analyzer_normal = self.parse_and_analyze(code, strict_types=False)
        warnings_normal = self.get_diagnostics_by_level(
            analyzer_normal, DiagnosticLevel.WARNING
        )
        errors_normal = self.get_diagnostics_by_level(
            analyzer_normal, DiagnosticLevel.ERROR
        )
        ...
        errors_strict = self.get_diagnostics_by_level(
            analyzer_strict, DiagnosticLevel.ERROR
        )
...
class TestSemanticAnalysisIntegration:
    ...
    def test_semantic_analysis_on_test_files(self, test_file):
        ...
        try:
            ...
            # Count diagnostics
            errors = [
                d for d in diagnostics.diagnostics if d.level == DiagnosticLevel.ERROR
            ]
            warnings = [
                d for d in diagnostics.diagnostics if d.level == DiagnosticLevel.WARNING
            ]
