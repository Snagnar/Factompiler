...
The compiler transforms FCDSL source code into Factorio blueprint strings through a 5-stage pipeline:

```
Source (.fcdsl) → Parser → Semantic Analyzer → AST Lowerer → Layout Planner → Blueprint Emitter → Blueprint
```
...
dsl_compiler/
├── grammar/
│   └── fcdsl.lark              # Lark grammar definition for the DSL
├── src/
...
def compile_source(source: str) -> str:
    """Compile FCDSL source code to blueprint string."""
...
# Example usage:
# source = Path("tests/sample_programs/01_basic_arithmetic.fcdsl").read_text()
