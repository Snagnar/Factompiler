...
class SymbolTable:
    ...
    def define(self, symbol: Symbol) -> None:
        ...
        if symbol.name in self.symbols:
            ...
            raise SemanticError(f"Symbol '{symbol.name}' already defined", symbol.defined_at)
...
class SemanticAnalyzer(ASTVisitor):
    ...
    def allocate_implicit_type(self) -> SignalTypeInfo:
        ...
        factorio_signal = f"signal-{chr(ord('A') + (self.implicit_type_counter - 1) % 26)}"
        ...
        if isinstance(expr, NumberLiteral):
            ...
            if expr.signal_type is None:
                ...
                if isinstance(expr.signal_type, StringLiteral):
                    ...
                    self.diagnostics.error("Signal type must be string literal", expr.signal_type)
            ...
            if object_symbol is None:
                ...
                if object_symbol.properties and expr.property_name in object_symbol.properties:
                    ...
                    self.diagnostics.error(f"Module '{expr.object_name}' has no function '{expr.property_name}'", expr)
                    return IntValue()
            else:
                self.diagnostics.error(f"Cannot access property '{expr.property_name}' on '{expr.object_name}' of type {object_symbol.symbol_type}", expr)
                return IntValue()
    ...
    def infer_binary_op_type(self, expr: BinaryOp) -> ValueInfo:
        ...
        if isinstance(left_type, IntValue) and isinstance(right_type, IntValue):
            ...
            warning_msg = (f"Mixed types in binary operation: "
            ...
            if left_type.signal_type.name == right_type.signal_type.name:
                ...
                warning_msg = (f"Mixed signal types in binary operation: "
    ...
    def infer_bundle_operation_type(self, left_type: ValueInfo, right_type: ValueInfo, expr: BinaryOp) -> BundleValue:
    ...
    def visit_FuncDecl(self, node: FuncDecl) -> None:
        ...
        for param_name in node.params:
            param_symbol = Symbol(
                ...
                value_type=SignalValue(signal_type=self.allocate_implicit_type()),
                defined_at=node
            )
            try:
...
def analyze_program(program: Program, strict_types: bool = False, analyzer: Optional['SemanticAnalyzer'] = None, file_path: Optional[str] = None) -> DiagnosticCollector:
