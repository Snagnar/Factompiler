...
# Alignment requirements for different entity types
# Most combinators are 1x1 (alignment=1), but some entities are 2x2 (alignment=2)
ENTITY_ALIGNMENT = {
    'train-stop': 2,
    'roboport': 2,
    'substation': 2,
...
class EntityPlacer:
    ...
    def _place_constant(self, op: IR_Const) -> None:
        ...
        signal_name = self.materializer.resolve_signal_name(
            op.output_type, usage
        )
        signal_type = self.materializer.resolve_signal_type(
            op.output_type, usage
        )
    ...
    def _place_user_entity(self, op: IR_PlaceEntity) -> None:
        ...
        # Determine footprint (simplified - would need full entity data in real impl)
        footprint = self._get_entity_footprint(prototype)
        
        # Determine alignment requirement
        alignment = ENTITY_ALIGNMENT.get(prototype, 1)
        ...
        if isinstance(op.x, int) and isinstance(op.y, int):
            ...
            pos = self.layout.get_next_position(footprint=footprint, alignment=alignment)
    ...
    def _get_entity_footprint(self, prototype: str) -> Tuple[int, int]:
        ...
        # Simplified lookup table for common entities
        FOOTPRINTS = {
            "train-stop": (2, 2),
            "roboport": (4, 4),
            "substation": (2, 2),
